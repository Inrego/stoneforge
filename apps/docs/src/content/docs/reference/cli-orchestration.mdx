...
When `--projectFilter` is set, the dispatch daemon only routes tasks from the
listed projects to this agent. Agents without a filter are **global** and
match tasks from any project (including workspace-scoped tasks that have no
`projectId`). Workspace-scoped tasks are never routed to filtered agents.
