 ...
 *
 * Multi-project element-ID collisions are resolved by the existing sync
 * merge strategy (import-order, last-writer-wins). Each project's imports
 * are scoped to that project's id so elements without an explicit
 * `projectId` in JSONL are attributed to the owning project — matching the
 * per-project sync routing established in el-3em.
 */
...
const PROJECT_SYNC_DIRNAME = '.stoneforge';
...
/** Absolute path to the global SQLite cache (`~/.stoneforge/stoneforge.db`). */
 ...
 * imported in registry order via the existing sync merge pipeline with each
 * project's `id` scoping the import.
 *
...
export function rebuildGlobalCache(input: RebuildInput): RebuildResult {
  ...
  try {
    ...
    for (const project of input.projects) {
      ...
      const inputDir = join(project.path, PROJECT_SYNC_DIRNAME);
      if (!existsSync(inputDir)) {
        // The project has no JSONL yet (newly-adopted workspace). That is
        // fine — just skip import for this project so the rebuild keeps
        // going; `syncService.importSync` would otherwise silently return
        // zero counts, but skipping is clearer for the reporting surface.
        skipped.push({
          id: project.id,
          name: project.name,
          path: project.path,
          reason: `Missing sync directory: ${inputDir}`,
        });
        continue;
      }

      const result = syncService.importSync({ inputDir, projectId: project.id });
...
function reasonToSkip(project: Project): string | null {
  try {
